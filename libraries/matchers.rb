...
if defined?(ChefSpec)
  ...
  action_list = %w(create add delete remove set reset refresh update configure reconfigure edit none discover apply reapply activate
                   stage new patch replace change load download upload extract backup enable disable allocate collect validate)
  ...
  oneview_resources = {
    ...
    oneview_id_pool:                    [:enable, :disable, :allocate_list, :allocate_count, :collect_ids, :validate],
