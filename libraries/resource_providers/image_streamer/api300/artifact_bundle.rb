    ...
    module API300
      ...
      class ArtifactBundleProvider < ResourceProvider
        ...
        def update_name
          ...
          new_name_in_use = resource_named(:ArtifactBundle).new(@item.client, name: @context.new_name).retrieve!
        ...
        def download
          ...
          @context.converge_by "Download #{@resource_name}'s '#{@name}.zip' to #{@context.file_path} completed successfully" do
        ...
        def upload
          ...
          return Chef::Log.info("#{@resource_name} #{@name} already exists in the appliance.") if @item.retrieve!
        ...
        def extract
          raise "#{@resource_name} #{@name} does not exist in the appliance. Cannot run extract action." unless @item.retrieve!
        ...
        def backup_from_file
          ...
          @context.converge_by "Upload of backup file '#{@context.file_path}' completed successfully" do
            @item.class.create_backup_from_file!(@item.client, deployment_group, @context.file_path, @name, @context.timeout)
          end
        end
