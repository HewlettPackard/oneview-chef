...
describe 'oneview_test_api300_c7000::ethernet_network_replace_scopes' do
  ...
  it 'replace scopes' do
    ...
    expect_any_instance_of(OneviewSDK::API300::C7000::EthernetNetwork).to receive(:replace_scopes).with([scope1, scope2])
  ...
  it 'does nothing when all scopes are already replaced' do
    allow_any_instance_of(OneviewSDK::API300::C7000::EthernetNetwork).to receive(:[]).with('scopeUris').and_return(['/rest/fake/2', '/rest/fake/1'])
    expect_any_instance_of(OneviewSDK::API300::C7000::EthernetNetwork).not_to receive(:replace_scopes)
    expect(real_chef_run).to replace_oneview_ethernet_network_scopes('EthernetNetwork1')
  end
end
