...
describe 'oneview_test::ethernet_network_reset_connection_template' do
  ...
  before do
    allow_any_instance_of(OneviewSDK::EthernetNetwork).to receive(:[]).and_call_original
    allow_any_instance_of(OneviewSDK::EthernetNetwork).to receive(:[]).with('connectionTemplateUri').and_return('/rest/fake-template/1')
