...
describe 'oneview_test_api300_synergy::fcoe_network_replace_scopes' do
  ...
  it 'replace scopes' do
    ...
    expect_any_instance_of(OneviewSDK::API300::Synergy::FCoENetwork).to receive(:replace_scopes).with([scope1, scope2])
  ...
  it 'does nothing when all scopes are already replaced' do
    allow_any_instance_of(OneviewSDK::API300::Synergy::FCoENetwork).to receive(:[]).with('scopeUris').and_return(['/rest/fake/2', '/rest/fake/1'])
    expect_any_instance_of(OneviewSDK::API300::Synergy::FCoENetwork).not_to receive(:replace_scopes)
    expect(real_chef_run).to replace_oneview_fcoe_network_scopes('FCoENetwork1')
  end
end
