...
describe 'oneview_test::logical_interconnect_group_create' do
  ...
  it 'updates it when it exists but not alike' do
    ...
    expect_any_instance_of(OneviewSDK::LogicalInterconnectGroup).to receive(:like?).and_return(false)
  ...
  it 'does nothing when it exists and is alike' do
    expect_any_instance_of(OneviewSDK::LogicalInterconnectGroup).to receive(:exists?).and_return(true)
    expect_any_instance_of(OneviewSDK::LogicalInterconnectGroup).to receive(:retrieve!).and_return(true)
    expect_any_instance_of(OneviewSDK::LogicalInterconnectGroup).to receive(:like?).and_return(true)
    expect_any_instance_of(OneviewSDK::LogicalInterconnectGroup).to_not receive(:update)
