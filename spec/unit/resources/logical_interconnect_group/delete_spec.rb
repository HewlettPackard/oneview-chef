...
describe 'oneview_test::logical_interconnect_group_delete' do
  ...
  it 'deletes it when it exists' do
    ...
    expect_any_instance_of(OneviewSDK::LogicalInterconnectGroup).to receive(:delete).and_return(true)
  ...
  it 'does nothing when it does not exist' do
    expect_any_instance_of(OneviewSDK::LogicalInterconnectGroup).to receive(:retrieve!).and_return(false)
    expect_any_instance_of(OneviewSDK::LogicalInterconnectGroup).to_not receive(:delete)
    expect(real_chef_run).to delete_oneview_logical_interconnect_group('LogicalInterconnectGroup3')
  end
end
