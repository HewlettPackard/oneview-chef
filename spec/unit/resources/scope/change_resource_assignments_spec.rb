...
describe 'oneview_test_api300_synergy::scope_change_resource_assignments' do
  ...
  let(:en_klass) { OneviewSDK::API300::C7000::Scope }
  let(:sh_klass) { OneviewSDK::API300::C7000::Scope }
  ...
  it 'Adds resources that do not exist and Removes resources that do exist when required' do
    ...
    allow_any_instance_of(en_klass).to receive(:[]).with('scopeUris').and_return([])
    allow_any_instance_of(sh_klass).to receive(:[]).with('scopeUris').and_return(['/rest/fake-scope-uri'])
