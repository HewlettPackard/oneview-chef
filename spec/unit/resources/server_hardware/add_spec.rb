...
describe 'oneview_test::server_hardware_add' do
  ...
  it 'adds it when it does not exist' do
    ...
    expect(real_chef_run).to add_oneview_server_hardware('ServerHardware1')
  end
  ...
  it 'updates it when it exists but not alike' do
    ...
    expect_any_instance_of(OneviewSDK::ServerHardware).to receive(:like?).and_return(false)
    expect_any_instance_of(OneviewSDK::ServerHardware).to receive(:update).and_return(true)
    expect(real_chef_run).to add_oneview_server_hardware('ServerHardware1')
  end
  ...
  it 'does nothing when it exists and is alike' do
    expect_any_instance_of(OneviewSDK::ServerHardware).to receive(:exists?).and_return(true)
    expect_any_instance_of(OneviewSDK::ServerHardware).to receive(:retrieve!).and_return(true)
    expect_any_instance_of(OneviewSDK::ServerHardware).to receive(:like?).and_return(true)
    expect_any_instance_of(OneviewSDK::ServerHardware).to_not receive(:update)
