...
describe 'oneview_test::storage_pool_add_with_ip' do
  ...
  it 'accepts the storage_system_ip parameter' do
    ...
    expect_any_instance_of(OneviewSDK::StoragePool).to receive(:exists?).and_return(false)
    expect_any_instance_of(OneviewSDK::StoragePool).to receive(:create).and_return(true)
