...
describe 'image_streamer_test_api300::artifact_bundle_delete' do
  ...
  it 'deletes it when it exists' do
    ...
    expect_any_instance_of(base_sdk::ArtifactBundle).to receive(:delete).and_return(true)
    expect(real_chef_run).to delete_image_streamer_artifact_bundle('ArtifactBundle1')
  end
  ...
  it 'does nothing when it does not exist' do
    ...
    expect_any_instance_of(base_sdk::ArtifactBundle).to_not receive(:delete)
    expect(real_chef_run).to delete_image_streamer_artifact_bundle('ArtifactBundle1')
  end
end
