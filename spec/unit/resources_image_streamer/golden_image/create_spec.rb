...
describe 'image_streamer_test_api300::golden_image_create' do
  ...
  it 'creates it when it does not exist' do
    ...
    expect(real_chef_run).to create_image_streamer_golden_image('GoldenImage1')
  end
  ...
  it 'updates it when it exists but not alike' do
    ...
    expect_any_instance_of(base_sdk::GoldenImage).to receive(:like?).and_return(false)
    expect_any_instance_of(base_sdk::GoldenImage).to receive(:update).and_return(true)
    expect(real_chef_run).to create_image_streamer_golden_image('GoldenImage1')
  end
  ...
  it 'does nothing when it exists and is alike' do
    expect_any_instance_of(base_sdk::GoldenImage).to receive(:exists?).and_return(true)
    expect_any_instance_of(base_sdk::GoldenImage).to receive(:retrieve!).and_return(true)
    expect_any_instance_of(base_sdk::GoldenImage).to receive(:like?).and_return(true)
    expect_any_instance_of(base_sdk::GoldenImage).to_not receive(:update)
